...
- Self-canonical URLs and 56 deterministic localized snapshots for released pages. Owner-published machine translations remain usable but `noindex` and stay out of locale sitemaps and reciprocal hreflang until a current-copy human approval records the reviewer, review date and service readiness. Intake/payment-return surfaces also stay noindex. Missing translations never become English content under a translated URL.
...
Search indexability is stricter than public release. An owner-authorized machine publication can appear in the language selector and complete the supported localized journey, but it produces `noindex` snapshots and no locale sitemap or reciprocal hreflang. Those discovery signals are enabled only by the separate `approved` path with real reviewer identity/date, current fingerprints and `serviceReady: true`.
...
For this release candidate, the eight English-source fingerprints were deliberately reconciled after the user-approved declined-offer copy was frozen. All seven existing publication records retain their original owner authorization, null reviewer fields and `serviceReady: false`. The 268-key catalogs and 1,876-row review CSV were unchanged. Validation confirms eight released interfaces, current review inventory and rejection of changed legal-source fingerprints; this is publication consistency evidence, not translation or legal review.
...
The August 31 legal/SEO follow-through refreshed the aggregate source fingerprint after the English Photo Radar insurer disclaimer changed. Bundle fingerprints, source-document hashes, the original owner authorization, null reviewer fields and `serviceReady: false` were preserved. The seven interfaces remain released but non-indexable; this refresh is not translation review.
