...
Updated 2026-09-04. **Brett authorized publishing and indexing all seven machine-translated interfaces without waiting for native or legal review.** They are public language options with intake and checkout enabled, a prominent versioned `NOT LEGAL ADVICE` / machine-translation notice, and the existing English agreements. No native or legal reviewer has approved these translations. Publication and indexing must not be represented as professional review or staffed native-language support.

The 2026-09-04 refresh qualifies the Photo Radar demerit statement in the shared FAQ and all seven bundles, removes three ambiguous Spanish negative predicates, restores the unilateral termination meaning in Punjabi, and binds the English resumable-intake lead consent to the exact-source gate. It preserves Brett's original publication and indexing provenance, null reviewer fields, and `serviceReady: false`.
...
English source references include `src/config/offers.json`, `src/pages/RapidResolution.tsx`, `src/pages/TermsOfService.tsx`, `src/components/TicketForm.tsx`, and `src/components/form-steps/{PersonalInfoStep,TicketDetailsStep,DefenseStep,ConsentStep,LeadCaptureFields,ReviewStep,PaymentStep}.tsx`. The integrated release preserves the original Rapid Resolution protections and adds English Photo Radar, Pro Driver and referral clauses in sections 5C–5E. Localized terms retain 16 selected sections; all eight catalogs now explicitly identify that limited coverage and direct users to the complete English agreement before authorization or purchase. The new product and program clauses are not represented as translated. A compact wording must still be compared with the complete English page during review.

The current release also preserves the distinct English `src/pages/TermsOfPurchase.tsx`, covering written-order precedence, existing-order terms and fee waivers, and the separation of consent from payment authorization. The translated service-term selections do not replace that document. Checkout acknowledges both English agreements and the Privacy Policy. The shared `LEGAL_SOURCE_DOCUMENT_PATHS` inventory in `src/i18n/locale-policy.mjs` requires exact hashes for all nine of these English sources before any non-English locale can be released:
...
The offer JSON is already part of the aggregate source fingerprint. The additional price-export and PDF files bind the values and authorization actually used by the agreements. Missing or changed sources invalidate publication even when their routes or documents remain English. English-only public product pages have separate source and SEO checks; the ordinary language-release gate does not approve those products for localized purchasing.
...
Both paths require the current source version, source and bundle fingerprints, and hashes for all nine English legal sources. The earlier 2026-08-31 integration deliberately refreshed these fingerprints after the offer exports, consent form and PDF source were frozen, while retaining Brett's original publication authorization and null native-review fields. The current 1,876-row CSV matches the same 268-key catalogs; no review evidence was invented. The later declined-offer clarification is explicitly English-only and adds the canonical refund JSON to the required source inventory. The resumable-intake lead consent is also an English fallback inside every localized intake, so it participates in the exact-source gate. After user-approved copy is frozen, its English-source fingerprints are deliberately reconciled without altering original owner authorization, reviewer fields or service readiness. Missing or changed fingerprints prevent publication until the changed copy is deliberately included in a new release record. Do not paste CSV SHA-256 values into implementation fingerprint fields, invent a reviewer, automatically refresh publication hashes during a build, or set `serviceReady` merely because a bundle compiles. Use `node scripts/validate-i18n.mjs --review-values` to inspect the current implementation fingerprints. A publication decision does not approve outgoing translated email templates, guarantee translation accuracy or prove backend deployment.
