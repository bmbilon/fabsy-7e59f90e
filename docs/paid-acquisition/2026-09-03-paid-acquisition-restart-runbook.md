...
5. **Verify production measurement.** With explicit consent, prove one row for each funnel event and its position/step. With decline and undecided states, prove optional Fabsy, Google and Meta measurement remains off. Confirm no form value, file, name, email, phone, IP, user agent or raw click ID reaches `paid_funnel_events`.
