...
- If the Crown offers a lower fine, fewer demerits, or both and the client declines it, the client does not qualify for a refund under the guarantee. Refunds under this policy apply only when Fabsy obtains no reduction on the client's behalf. This clarification does not authorize accepting an offer without the client's case-specific instruction.
