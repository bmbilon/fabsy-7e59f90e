...
This activation supersedes the earlier no-tag/deployment-pending status below. It does **not** verify actual paid-purchase ingestion, matching, value/deduplication or attribution, and does not authorize advertising. The receipt preserves its earlier capture-guard/publication observations. Those observations do not establish publication of this new Crown-rejection timing correction, which remains pending. The advertising budget and total test cap are unapproved.
...
The existing Rapid Resolution label remains `MyAbCPiLj-scEPmV_s5E`; camera purchases must not use it. These inputs are retained in the historical activation receipt; actual paid delivery to the Photo Radar action is still unverified:
...
The earlier release-owner report recorded these identifiers in GitHub secrets and the Photo label in both CI build environments, with build/deployment and paid receipt still pending at that time. Those historical values remain unchanged in `launch-settings.json` and the validation report. The separate activation receipt documents publication and Google measurement activation of that earlier release; it leaves actual paid receipt and attribution unresolved. No configuration change or repeated setup approval is requested here.
...
The revised offer charges **$79 + GST ($82.95 total) upfront**, with **no hidden fees**. If the Crown rejects Fabsy's efforts to reduce the original fine or obtain a withdrawal of your photo radar or red-light camera notice, and neither improvement is obtained, Fabsy refunds your service fee within 30 days of receiving the rejection. These notices have no demerits. The refund covers the actual Photo Radar service fee and GST paid. Payment does not start the 30-day refund clock. An initial or unchanged Crown offer alone does not trigger a refund. This is not a guaranteed legal outcome. The refund trigger requires rejection of Fabsy's efforts and neither a lower original fine nor withdrawal obtained, without a final-offer requirement, minimum reduction, claim deadline or guilty-plea acceptance condition. It is separate from the final approved outcome used in the cohort analysis below. Account for refunded GST separately from net service revenue. Include these service-fee refunds in revenue/contribution reconciliation; they do not change the original paid-purchase value of CAD 79 excluding GST or the government fine. This copy revision is not evidence that a refund or conversion adjustment has been processed.
