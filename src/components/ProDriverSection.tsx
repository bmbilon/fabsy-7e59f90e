  ...
  return (
    ...
    >
      <div className="container mx-auto grid max-w-6xl gap-8 lg:grid-cols-[1.2fr_0.8fr] lg:items-center lg:gap-12">
        ...
        <div className="rounded-2xl border border-blue-200 bg-white p-6 shadow-fab sm:p-7">
          <p className="text-sm text-slate-500">{t('proDriver.regularPrice')} <s dir="ltr">${RAPID_RESOLUTION.priceCad} CAD</s></p>
          <p className="mt-4 text-sm font-semibold text-primary-dark">{t('proDriver.discountedPrice')}</p>
          <p className="mt-2 text-4xl font-bold tracking-tight text-slate-950" dir="ltr">${PRO_DRIVER_PRICE_CAD.toFixed(2)} <span className="text-base font-medium text-slate-600">CAD + GST</span></p>
          <p className="mt-3 text-sm leading-6 text-slate-600">{isolatePrices(t('proDriver.savings'))}</p>
          <p className="mt-3 text-sm font-medium leading-6 text-slate-700">{isolatePrices(t('proDriver.bundlePrice'))}</p>
          <Button asChild size="lg" className="mt-6 h-auto min-h-12 w-full whitespace-normal bg-primary-dark py-3 text-center text-white hover:bg-primary-dark/90 hover:text-white focus-visible:text-white">
            <Link to={PRO_DRIVER_PROMOTION.detailsPath}>
              {t('proDriver.cta')}<ArrowRight className="ms-2 h-5 w-5 shrink-0 rtl:rotate-180" aria-hidden="true" />
            </Link>
          </Button>
          <p className="mt-4 text-sm leading-6 text-slate-600">{t('proDriver.claimHint')}</p>
          {locale !== 'en' && <p className="mt-3 text-sm leading-6 text-slate-600">{t('proDriver.englishDetails')}</p>}
        </div>
      </div>
    </section>
  );
