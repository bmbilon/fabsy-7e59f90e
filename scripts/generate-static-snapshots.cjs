...
${page.sources.map((source) => `          <li><a href="${esc(source.url)}" rel="noopener noreferrer">${esc(source.title)}</a></li>`).join('\n')}
  ...
  const offerHtml = photoRadar
    ? `<section class="triage" aria-labelledby="photo-radar-heading"><h2 id="photo-radar-heading">Photo radar or red-light camera notice?</h2><p>${esc(PHOTO_RADAR_PRICING_COPY)}</p><p>${esc(OFFERS.photoRadar.insuranceDisclaimer)} You approve any deal.</p><p>${esc(OFFERS.photoRadar.speedDisclaimer)}</p><p><a class="cta" href="/photo-radar">Review Rapid Resolution: Photo Radar</a></p></section>`
