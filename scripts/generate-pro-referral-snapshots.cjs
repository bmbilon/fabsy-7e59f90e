...
<section><p>${esc(copy.badge)}</p><h1>${esc(copy.heading)}</h1><p>${esc(copy.intro)}</p><aside data-fee-refund-notice="ticket-representation"><h2>${esc(feeRefund.headline)}</h2><p>${esc(feeRefund.condition)}</p><p lang="en" dir="ltr">${esc(feeRefund.declinedOfferText)}</p><p>${esc(feeRefund.payment)}</p><p><a href="${esc(feeRefund.termsPath)}">${esc(feeRefund.details)}</a></p></aside><a class="cta" href="${esc(intake)}">Upload your ticket</a><p>${esc(copy.scope)}</p>
