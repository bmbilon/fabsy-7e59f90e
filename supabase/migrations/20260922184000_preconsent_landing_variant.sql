-- Aggregate counts only; no new identifier, cookie or campaign payload.
begin;
alter table analytics_private.preconsent_metrics_hourly drop constraint preconsent_metric_page_check;
alter table analytics_private.preconsent_metrics_hourly add constraint preconsent_metric_page_check check (page_key in ('home','rapid_resolution','rapid_resolution_alt','photo_radar','pro_drivers'));
...
begin
  if p_event_name not in ('paid_landing','consent_accepted','consent_declined','consent_dismissed') or
      p_page_key not in ('home','rapid_resolution','rapid_resolution_alt','photo_radar','pro_drivers') or
