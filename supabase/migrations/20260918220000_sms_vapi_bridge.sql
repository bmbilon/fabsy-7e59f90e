...
create table public.sms_vapi_conversations (
...
create table public.sms_vapi_messages (
...
create table public.sms_vapi_runtime (
...
insert into public.sms_vapi_runtime(singleton_id)
values (1);

create index sms_vapi_messages_sender_received_idx
  on public.sms_vapi_messages(sender_hash, received_at desc);

create index sms_vapi_messages_delivery_pending_idx
  on public.sms_vapi_messages(received_at)
  where outbound_message_sid is not null and delivery_status is null;

create index sms_vapi_messages_retention_idx
  on public.sms_vapi_messages(retained_until);

alter table public.sms_vapi_conversations enable row level security;
alter table public.sms_vapi_messages enable row level security;
alter table public.sms_vapi_runtime enable row level security;

revoke all on public.sms_vapi_conversations from public, anon, authenticated;
revoke all on public.sms_vapi_messages from public, anon, authenticated;
revoke all on public.sms_vapi_runtime from public, anon, authenticated;

grant select on public.sms_vapi_conversations to authenticated;
grant select on public.sms_vapi_messages to authenticated;
grant select on public.sms_vapi_runtime to authenticated;

create policy "Staff can read SMS conversation metadata"
  on public.sms_vapi_conversations
  for select
  to authenticated
  using (
...
create policy "Staff can read SMS message metadata"
  on public.sms_vapi_messages
  for select
  to authenticated
  using (
    public.has_role(auth.uid(), 'admin'::public.app_role)
...
create policy "Staff can read SMS runtime status"
  on public.sms_vapi_runtime
  for select
  to authenticated
  using (
    public.has_role(auth.uid(), 'admin'::public.app_role)
...
comment on table public.sms_vapi_conversations is
  'Minimal SMS/Vapi continuity metadata. Sender identity is stored only as an HMAC-SHA-256 pseudonym; no phone number or message content is stored.';
comment on table public.sms_vapi_messages is
...
comment on table public.sms_vapi_runtime is
...
comment on column public.sms_vapi_conversations.previous_chat_id is
  'Latest Vapi Chat API id used as previousChatId for the next inbound message.';
comment on column public.sms_vapi_conversations.assistant_id is
  'Assistant binding for previous_chat_id; switching assistants invalidates prior chat continuity.';
comment on column public.sms_vapi_conversations.opted_out_at is
  'Suppresses automated replies until an explicit START/UNSTOP control message clears it.';
comment on column public.sms_vapi_conversations.latest_control_message_sid is
  'Serializes STOP/START authority by atomic claim order so delayed completions cannot reverse newer control state.';
comment on column public.sms_vapi_messages.control_action is
  'Control metadata only: true is STOP, false is START, and null is an ordinary message.';
comment on column public.sms_vapi_messages.retained_until is
  'Metadata retention deadline. The migration schedules purge_sms_vapi_metadata daily through pg_cron.';

create or replace function public.claim_sms_vapi_inbound(
  p_message_sid text,
  p_sender_hash text,
...
declare
  ...
  conversation public.sms_vapi_conversations%rowtype;
...
begin
  if p_message_sid is null or p_message_sid !~ '^SM[0-9A-Fa-f]{32}$' then
    raise exception 'SMS_MESSAGE_SID_INVALID' using errcode = '22023';
  end if;
  if p_sender_hash is null or p_sender_hash !~ '^[0-9a-f]{64}$' then
    raise exception 'SMS_SENDER_HASH_INVALID' using errcode = '22023';
  end if;
  if p_previous_sender_hash is not null and
     (p_previous_sender_hash !~ '^[0-9a-f]{64}$' or p_previous_sender_hash = p_sender_hash) then
    raise exception 'SMS_PREVIOUS_SENDER_HASH_INVALID' using errcode = '22023';
  end if;
  if p_body_length is null or p_body_length < 0 or p_body_length > 100000 then
    raise exception 'SMS_BODY_LENGTH_INVALID' using errcode = '22023';
  end if;
  if p_num_media is null or p_num_media < 0 or p_num_media > 10 then
    raise exception 'SMS_MEDIA_COUNT_INVALID' using errcode = '22023';
  end if;
  if p_assistant_id is null then
    raise exception 'SMS_ASSISTANT_ID_INVALID' using errcode = '22023';
  end if;
  if p_rate_limit is null or p_rate_limit < 1 or p_rate_limit > 100 then
    raise exception 'SMS_RATE_LIMIT_INVALID' using errcode = '22023';
  end if;

  select * into conversation
  from public.sms_vapi_conversations
  ...
  if conversation_found and p_previous_sender_hash is not null then
    ...
    if found then
      raise exception 'SMS_SENDER_HASH_ROTATION_CONFLICT' using errcode = '23505';
    end if;
  end if;
  ...
  if not conversation_found and p_previous_sender_hash is not null then
    ...
    if conversation_found then
      update public.sms_vapi_conversations
  ...
  if not conversation_found then
    insert into public.sms_vapi_conversations(sender_hash, assistant_id)
  ...
  if not conversation_found then
    raise exception 'SMS_CONVERSATION_CLAIM_FAILED' using errcode = '40001';
  end if;

  insert into public.sms_vapi_messages(
  ...
  if inserted_sid is null then
    ...
    from public.sms_vapi_messages
    where message_sid = p_message_sid;
    if not found or duplicate_sender_hash <> p_sender_hash then
      raise exception 'SMS_MESSAGE_SID_CONFLICT' using errcode = '23505';
    end if;
...
create or replace function public.authorize_sms_vapi_chat(
  p_message_sid text,
  p_sender_hash text,
...
declare
  ...
  runtime public.sms_vapi_runtime%rowtype;
  operation_at timestamptz := clock_timestamp();
...
begin
  ...
  if p_limit_per_ten_minutes is null or
    ...
    raise exception 'SMS_GLOBAL_TEN_MINUTE_LIMIT_INVALID' using errcode = '22023';
  end if;
  if p_limit_per_day is null or p_limit_per_day < 1 or p_limit_per_day > 10000 then
    raise exception 'SMS_GLOBAL_DAILY_LIMIT_INVALID' using errcode = '22023';
  end if;
  ...
  for update;
  if not found then
    raise exception 'SMS_CONVERSATION_NOT_FOUND' using errcode = 'P0002';
  end if;
  ...
  if not found or message_state <> 'processing' then
    raise exception 'SMS_MESSAGE_NOT_AUTHORIZABLE' using errcode = '55000';
  end if;
  ...
  if conversation.opted_out_at is not null or
    ...
    select * into runtime
    from public.sms_vapi_runtime
    where singleton_id = 1
    for update;
    if not found then
      raise exception 'SMS_RUNTIME_NOT_FOUND' using errcode = 'P0002';
    end if;
    ...
    update public.sms_vapi_runtime
  ...
  update public.sms_vapi_messages
...
create or replace function public.set_sms_vapi_circuit_enabled(
  p_enabled boolean
)
returns boolean
language plpgsql
...
begin
  if p_enabled is null then
    raise exception 'SMS_CIRCUIT_STATE_INVALID' using errcode = '22023';
  end if;
...
create or replace function public.complete_sms_vapi_inbound(
  p_message_sid text,
  p_sender_hash text,
  p_state text,
  p_vapi_chat_id text,
...
begin
  ...
  if p_state is null or p_state not in (
    ...
    raise exception 'SMS_COMPLETION_STATE_INVALID' using errcode = '22023';
  end if;
  if p_vapi_chat_id is not null and length(p_vapi_chat_id) not between 1 and 200 then
    raise exception 'SMS_VAPI_CHAT_ID_INVALID' using errcode = '22023';
  end if;
  if p_state = 'replied' and p_vapi_chat_id is null then
    raise exception 'SMS_VAPI_CHAT_ID_REQUIRED' using errcode = '22023';
  end if;
  if p_reply_length is null or p_reply_length < 0 or p_reply_length > 100000 then
    raise exception 'SMS_REPLY_LENGTH_INVALID' using errcode = '22023';
  end if;
  ...
  set previous_chat_id = case
      ...
      previous_chat_expires_at = case
        when reply_allowed and p_reset_chat and processing_message_sid = p_message_sid then null
        when reply_allowed and p_state = 'replied' then clock_timestamp() + interval '24 hours'
        else previous_chat_expires_at
      end,
      opted_out_at = case
...
create or replace function public.record_sms_vapi_status(
...
begin
  if p_inbound_message_sid is null or p_inbound_message_sid !~ '^SM[0-9A-Fa-f]{32}$' or
    ...
    raise exception 'SMS_STATUS_SID_INVALID' using errcode = '22023';
  end if;
  ...
  if p_error_code is not null and p_error_code !~ '^\d{1,10}$' then
    raise exception 'SMS_DELIVERY_ERROR_INVALID' using errcode = '22023';
  end if;
...
create or replace function public.purge_sms_vapi_metadata()
returns jsonb
language plpgsql
security definer
set search_path = pg_catalog, pg_temp
...
declare
  ...
  operation_at timestamptz := clock_timestamp();
begin
  delete from public.sms_vapi_messages
  ...
  delete from public.sms_vapi_conversations as conversation
  where conversation.opted_out_at is null
    ...
    and not exists (
      select 1
      from public.sms_vapi_messages as message
...
begin
  for existing_job_id in
    select jobid
    from cron.job
    where jobname = 'fabsy-sms-vapi-metadata-purge'
  loop
    perform cron.unschedule(existing_job_id);
  end loop;

  perform cron.schedule(
    'fabsy-sms-vapi-metadata-purge',
    '17 3 * * *',
    'select public.purge_sms_vapi_metadata();'
  );
end;
$sms_purge_schedule$;
...
begin
  ...
  if result ->> 'busy' <> 'false' or result ->> 'opted_out' <> 'true' then
    raise exception 'SMS_MIGRATION_ASSERT_STOP_CLAIM';
  end if;

  select public.complete_sms_vapi_inbound(
    stop_sid, test_sender_hash, 'opted_out', null, 30, true, true
  ) into result;
  if result ->> 'reply_allowed' <> 'true' then
    raise exception 'SMS_MIGRATION_ASSERT_STOP_ALLOWED';
  end if;
  ...
  if result ->> 'completed' <> 'true' or result ->> 'reply_allowed' <> 'false' then
    raise exception 'SMS_MIGRATION_ASSERT_STOP_IDEMPOTENCE';
  end if;
  ...
  if result ->> 'opted_out' <> 'false' then
    raise exception 'SMS_MIGRATION_ASSERT_START_CLAIM';
  end if;
  ...
  if result ->> 'reply_allowed' <> 'true' then
    raise exception 'SMS_MIGRATION_ASSERT_START_ALLOWED';
  end if;
  ...
  if result ->> 'reply_allowed' <> 'false' then
    raise exception 'SMS_MIGRATION_ASSERT_STALE_REPLY_SUPPRESSED';
  end if;
  ...
  if stored_chat_id is not null or stored_opted_out_at is not null then
    raise exception 'SMS_MIGRATION_ASSERT_STALE_STATE_RESTORED';
  end if;

  perform public.record_sms_vapi_status(
  ...
  if stored_status <> 'delivered' then
    raise exception 'SMS_MIGRATION_ASSERT_STATUS_REGRESSION';
  end if;

  -- A delayed older START cannot reverse a newer STOP.
  perform public.claim_sms_vapi_inbound(
  ...
  if result ->> 'reply_allowed' <> 'false' then
    raise exception 'SMS_MIGRATION_ASSERT_STALE_START_ALLOWED';
  end if;
  select public.complete_sms_vapi_inbound(
    old_start_sid, start_then_stop_hash, 're_enabled', null, 30, false, true
  ) into result;
  if result ->> 'completed' <> 'true' or result ->> 'reply_allowed' <> 'false' then
    raise exception 'SMS_MIGRATION_ASSERT_STALE_START_NOT_IDEMPOTENT';
  end if;
  ...
  if stored_opted_out_at is null or stored_latest_control_sid <> new_stop_sid then
    raise exception 'SMS_MIGRATION_ASSERT_STALE_START_REVERSED_STOP';
  end if;
  ...
  if result ->> 'reply_allowed' <> 'false' then
    raise exception 'SMS_MIGRATION_ASSERT_STALE_STOP_ALLOWED';
  end if;
  select public.complete_sms_vapi_inbound(
    old_stop_sid, stop_then_start_hash, 'opted_out', null, 30, true, true
  ) into result;
  if result ->> 'completed' <> 'true' or result ->> 'reply_allowed' <> 'false' then
    raise exception 'SMS_MIGRATION_ASSERT_STALE_STOP_NOT_IDEMPOTENT';
  end if;
  select opted_out_at, latest_control_message_sid
  into stored_opted_out_at, stored_latest_control_sid
  from public.sms_vapi_conversations
  ...
  if stored_opted_out_at is not null or stored_latest_control_sid <> new_start_sid then
    raise exception 'SMS_MIGRATION_ASSERT_STALE_STOP_REVERSED_START';
  end if;
  ...
  perform public.set_sms_vapi_circuit_enabled(true);
  perform public.claim_sms_vapi_inbound(
    global_allowed_sid, global_allowed_hash, 5, 0,
    test_assistant_id, null, null, 10
  );
  select public.authorize_sms_vapi_chat(
    global_allowed_sid, global_allowed_hash, 1, 2
  ) into result;
  if result ->> 'allowed' <> 'true' then
    raise exception 'SMS_MIGRATION_ASSERT_GLOBAL_FIRST_DENIED';
  end if;
  perform public.authorize_sms_vapi_chat(
    global_allowed_sid, global_allowed_hash, 1, 2
  );
  ...
  if stored_count <> 1 then
    raise exception 'SMS_MIGRATION_ASSERT_GLOBAL_NOT_IDEMPOTENT';
  end if;
  ...
  if result ->> 'reason' <> 'ten_minute_limit' then
    raise exception 'SMS_MIGRATION_ASSERT_GLOBAL_TEN_MINUTE_LIMIT';
  end if;

  perform public.set_sms_vapi_circuit_enabled(false);
  perform public.claim_sms_vapi_inbound(
  ...
  if result ->> 'reason' <> 'disabled' then
    raise exception 'SMS_MIGRATION_ASSERT_GLOBAL_CIRCUIT';
  end if;
  ...
  if result ->> 'reason' <> 'daily_limit' then
    raise exception 'SMS_MIGRATION_ASSERT_GLOBAL_DAILY_LIMIT';
  end if;
  ...
  perform public.purge_sms_vapi_metadata();
  if exists (
    select 1 from public.sms_vapi_conversations
    where sender_hash = purge_stale_hash
  ) or exists (
    select 1 from public.sms_vapi_messages
    where message_sid = purge_stale_sid
  ) then
    raise exception 'SMS_MIGRATION_ASSERT_EXPIRED_LEASE_RETAINED';
  end if;
  ...
  delete from public.sms_vapi_conversations
  where sender_hash in (
    test_sender_hash, start_then_stop_hash, stop_then_start_hash,
    global_allowed_hash, global_limited_hash, global_disabled_hash,
    global_daily_hash, purge_stale_hash
...
$sms_migration_assertions$;

revoke all on function public.claim_sms_vapi_inbound(text, text, integer, integer, uuid, boolean, text, integer)
  from public, anon, authenticated;
revoke all on function public.complete_sms_vapi_inbound(text, text, text, text, integer, boolean, boolean)
  from public, anon, authenticated;
revoke all on function public.authorize_sms_vapi_chat(text, text, integer, integer)
  from public, anon, authenticated;
revoke all on function public.set_sms_vapi_circuit_enabled(boolean)
  from public, anon, authenticated;
revoke all on function public.record_sms_vapi_status(text, text, text, text)
  from public, anon, authenticated;
revoke all on function public.purge_sms_vapi_metadata()
  from public, anon, authenticated;

grant execute on function public.claim_sms_vapi_inbound(text, text, integer, integer, uuid, boolean, text, integer)
  to service_role;
grant execute on function public.complete_sms_vapi_inbound(text, text, text, text, integer, boolean, boolean)
  to service_role;
grant execute on function public.authorize_sms_vapi_chat(text, text, integer, integer)
  to service_role;
grant execute on function public.set_sms_vapi_circuit_enabled(boolean)
  to service_role;
grant execute on function public.record_sms_vapi_status(text, text, text, text)
  to service_role;
grant execute on function public.purge_sms_vapi_metadata()
  to service_role;
