-- Admit consented Photo Radar landing checkpoints through the same bounded
-- public-page contract as Rapid Resolution. Consent, webhook-only purchases,
-- private intake pages, reports and cash facts are unchanged. Public checkpoint
-- deduplication is per session and page so visits to both offers are retained.
