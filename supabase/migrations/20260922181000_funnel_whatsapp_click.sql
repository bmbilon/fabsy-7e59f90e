-- Add WhatsApp clicks to the existing consented action contract.
begin;
...
alter table analytics_private.paid_funnel_events
  add constraint paid_funnel_event_name_check check (event_name in (
    ...
    'engaged_10s','engaged_30s','engaged_60s','scroll_25','scroll_50','scroll_75','scroll_90',
    'intake_started','intake_step_viewed','intake_validation_blocked','ticket_upload_started',
    'ticket_upload_failed','ticket_uploaded','lead_saved','intake_step_completed',
    'checkout_started','checkout_canceled','purchase'
  )),
  add constraint paid_funnel_position_check check (
    (event_name in ('primary_cta_viewed','primary_cta_click','phone_click','whatsapp_click') and
      (position is null or position in ('hero','header','sticky','section','footer')))
    or (event_name not in ('primary_cta_viewed','primary_cta_click','phone_click','whatsapp_click') and position is null)
  );

alter table analytics_private.paid_funnel_events
  drop constraint paid_funnel_page_key_check,
  drop constraint paid_funnel_event_page_check;

alter table analytics_private.paid_funnel_events
  add constraint paid_funnel_page_key_check check (page_key in (
    'rapid_resolution','photo_radar','intake','payment_canceled','thank_you'
  )),
  add constraint paid_funnel_event_page_check check (
    ...
    ) and page_key in ('rapid_resolution','photo_radar'))
    or (event_name in (
...
-- Public checkpoints may occur on both offers in one session. Reports retain
-- their existing distinct-session aggregates; event_count counts both pages.
-- Private events keep a single permitted page, preserving their deduplication.
...
create unique index paid_funnel_session_singleton_idx
  on analytics_private.paid_funnel_events(session_id, page_key, event_name)
  where event_name not in (
    'primary_cta_viewed','primary_cta_click','phone_click','whatsapp_click',
    'intake_step_viewed','intake_validation_blocked','intake_step_completed'
  );
create unique index paid_funnel_session_action_idx
  on analytics_private.paid_funnel_events(session_id, page_key, event_name, coalesce(position, ''))
  where event_name in ('primary_cta_viewed','primary_cta_click','phone_click','whatsapp_click');
...
begin
  ...
  if p_page_key not in ('rapid_resolution','photo_radar','intake','payment_canceled','thank_you') then
  ...
  return inserted_count = 1 or exists (
    ...
    where session_id = p_session_id and page_key = p_page_key and event_name = p_event_name and (
      event_id = p_event_id
      or (p_event_name in ('primary_cta_viewed','primary_cta_click','phone_click','whatsapp_click') and position is not distinct from p_position)
