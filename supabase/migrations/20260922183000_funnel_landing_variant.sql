-- Keep control and challenger events distinct, including within one consented session.
begin;
...
alter table analytics_private.paid_funnel_events
  add constraint paid_funnel_page_key_check check (page_key in (
    'rapid_resolution','rapid_resolution_alt','photo_radar','intake','payment_canceled','thank_you'
  )),
  add constraint paid_funnel_event_page_check check (
    ...
    ) and page_key in ('rapid_resolution','rapid_resolution_alt','photo_radar'))
    or (event_name in (
...
begin
  ...
  if p_page_key not in ('rapid_resolution','rapid_resolution_alt','photo_radar','intake','payment_canceled','thank_you') then
