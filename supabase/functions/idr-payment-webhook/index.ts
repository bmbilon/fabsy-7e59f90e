...
async function sendTicketAssessmentConfirmation(
  ...
  try {
    ...
    await sendResendEmail(apiKey, {
      ...
      to: [client.email],
      subject: "Fabsy received your Ticket Triage order",
      html: `<div style="font-family:Arial,sans-serif;max-width:640px;margin:0 auto;color:#1f2937"><h1>Payment received—your Ticket Triage is in the review queue</h1><p>Hi ${escapeHtml(client.first_name)},</p><p>Fabsy received your private ticket upload and Ticket Triage information.</p><div style="background:#f5f3ff;border:1px solid #ddd6fe;border-radius:10px;padding:18px;margin:24px 0"><p style="margin:0 0 8px"><strong>Payment:</strong> $149 CAD, one-time, plus GST</p><p style="margin:0 0 8px"><strong>Ticket:</strong> received</p><p style="margin:0"><strong>Next:</strong> a Fabsy team member will complete a human review and email your Ticket Triage result.</p></div><div style="background:#f5f3ff;border:1px solid #c4b5fd;border-radius:10px;padding:18px;margin:24px 0"><p style="margin:0 0 8px"><strong>Your upgrade benefits:</strong> priority placement in Fabsy's representation queue for the same eligible matter.</p><p style="margin:0">Your $149 Ticket Triage payment is credited toward the $488 flat representation fee, leaving a $339 base-fee balance plus GST. The 30% success fee still applies to any fine reduction.</p></div><p>Fabsy will review the charge and deadline, fine and demerit implications, available options, likely insurance-risk significance, representation economics and the recommended next step.</p><p>If a response deadline is close, reply to this email or call (825) 793-2279 after submitting. Ticket Triage does not pause any deadline printed on the ticket.</p><p style="font-size:13px;color:#6b7280;line-height:1.5">Insurance treatment varies by insurer, driving history, jurisdiction, renewal timing and other underwriting factors. Ticket Triage is not a binding insurance quote or guarantee of premium changes. Fabsy is an Alberta traffic ticket agent service, not a law firm, and no outcome is promised.</p>${getFabsyEmailSignature()}</div>`,
