  ...
  try {
    ...
    try {
      const representationPath = result.representation_recommended
        ? `<div style="margin-top:24px;padding:18px;border:1px solid #ddd6fe;border-radius:10px;background:#f5f3ff"><h2 style="margin-top:0">Fabsy action path</h2><p>${escapeHtml(result.next_step)}</p><p><a href="mailto:hello@fabsy.ca?subject=Assessment%20representation%20upgrade" style="display:inline-block;background:#7c3aed;color:#fff;padding:11px 18px;border-radius:6px;text-decoration:none;font-weight:700">Ask about representation</a></p><p style="font-size:13px;color:#6b7280">Because representation is recommended and this matter is eligible, your $149 assessment payment can be applied to Fabsy's $488 base representation fee, leaving a $339 base-fee balance plus applicable tax. Eligible assessment clients also receive priority placement. The 30% success fee still applies to any fine reduction.</p></div>`
