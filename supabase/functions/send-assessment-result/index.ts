...
serve(async (req) => {
  ...
  try {
    ...
    if (submission.service_type !== "ticket_insurance_assessment" || !submission.assessment_paid_at || !submission.representation_credit_eligible) {
      return json({ error: "Only paid Ticket Triage orders can be delivered." }, 409);
    }
    ...
    try {
      const representationPath = result.representation_recommended
        ? `<div style="margin-top:24px;padding:18px;border:1px solid #ddd6fe;border-radius:10px;background:#f5f3ff"><h2 style="margin-top:0">Fabsy action path</h2><p>${escapeHtml(result.next_step)}</p><p><a href="mailto:hello@fabsy.ca?subject=Ticket%20Triage%20representation%20upgrade" style="display:inline-block;background:#7c3aed;color:#fff;padding:11px 18px;border-radius:6px;text-decoration:none;font-weight:700">Activate my priority upgrade</a></p><p style="font-size:13px;color:#6b7280">You receive priority placement in Fabsy's representation queue for this eligible matter. Your $149 Ticket Triage payment is credited toward the $488 flat fee, leaving a $339 base-fee balance plus GST. The 30% success fee still applies to any fine reduction.</p></div>`
      ...
      await sendResendEmail(resendApiKey, {
        ...
        to: [client.email],
        subject: "Your Fabsy Ticket Triage result",
