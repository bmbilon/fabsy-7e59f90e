...
serve(async (req) => {
  ...
  try {
    ...
    const { data: submission, error: submissionError } = await admin.from("ticket_submissions")
      .select("id,status,service_type,assessment_paid_at,assessment_result,assessment_delivered_at,assessment_delivery_claimed_at,assessment_delivery_sent_at,clients(first_name,email)")
      .eq("id", submissionId).single();
    if (submissionError || !submission) return json({ error: "Assessment submission was not found." }, 404);
    if (submission.service_type !== "ticket_insurance_assessment" || !submission.assessment_paid_at) {
      return json({ error: "Only paid ticket assessments can be delivered." }, 409);
    }
    ...
    try {
      const representationPath = result.representation_recommended
        ? `<div style="margin-top:24px;padding:18px;border:1px solid #ddd6fe;border-radius:10px;background:#f5f3ff"><h2 style="margin-top:0">Fabsy action path</h2><p>${escapeHtml(result.next_step)}</p><p><a href="https://fabsy.ca/submit-ticket" style="display:inline-block;background:#7c3aed;color:#fff;padding:11px 18px;border-radius:6px;text-decoration:none;font-weight:700">Ask Fabsy about representation</a></p><p style="font-size:13px;color:#6b7280">Eligibility and pricing are confirmed separately. The $149 assessment fee is not currently promised as a representation credit.</p></div>`
