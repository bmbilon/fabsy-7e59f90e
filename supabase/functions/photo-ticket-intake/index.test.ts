...
Deno.test("contact and offer context are supplied before upload without setting authoritative ticket type", async () => {
  await boundary(async state => {
    const response = await handler(request({ ...prepare, email: "Alex@Example.test", ticketType: "photo_radar", landingPage: "rapid-resolution-alt", bundleRequested: false }));
    assert.equal(response.status, 200);
    assert.equal((await response.json()).contactSaved, true);
    assert.equal(state.row.email, "alex@example.test");
    assert.equal(state.row.intake_ticket_type, "photo_radar");
    assert.equal(state.row.ticket_type, "photo_radar");
    assert.equal(state.row.intake_review_status, "pending_scan", "selection still requires scan confirmation before checkout");
    assert.equal(state.row.landing_page_variant, "rapid-resolution-alt");
