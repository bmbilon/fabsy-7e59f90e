...
This runbook covers the multilingual schema and function deployment. Deploying them does not connect n8n or Claude, send a customer message, or approve a language for sales. The checked-in intake path contains no translator connection; the external worker remains separate work. Keep translation review and service-readiness gates closed until their requirements are met.

## Release sequence

1. Apply `migrations/20260830160000_multilingual_intake.sql` before deploying the changed functions. Existing records receive `preferred_locale = 'en'`. The migration deliberately does not enqueue historical client notes.
2. Deploy the changed intake, payment, webhook and notification functions together with `_shared/locale-policy.ts`, `_shared/notification-locale.ts` and `_shared/resend-email.ts`. Deploy `generate-consent-form` with the PDF helpers and all its declared local static assets using the CLI/Docker bundler; see the consent PDF section below. Do not deploy that function through the dashboard or API bundler.
3. Keep `FABSY_REVIEWED_SERVICE_LOCALES` unset until the same frontend source and locale bundle have documented native/legal review and the advertised contact channel is staffed. The frontend review manifest is `src/i18n/review-status.json`; its fingerprint checks and `serviceReady` flag must also pass. Then an operator can attest the released codes in the server secret, for example `pa,tl`. This value is never populated automatically. Remove a code whenever its source/bundle approval or service readiness changes. A backend secret alone is not translation approval.
...
Each row starts `pending`. This means **waiting for an external translation worker**, not “translated” or “translation in progress.” No worker, scheduled job or external provider call is installed by this change. The raw intake remains available to staff throughout; a pending translation does not determine payment or case outcome.
